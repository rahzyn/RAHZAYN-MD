...
let auto_reply_message = "Hello 👋,here an QART_MD my owner is unavailable. Kindly leave a message.";
...
setTimeout(() => {
  async function main() {
    ...
    zk.ev.on("call", async (callData) => {
      if (conf.ANTI_CALL === 'yes') {
        ...
        await zk.sendMessage(callerId, {
          text: "❌ MY OWNER IS NOT AVAILABLE FOR NOW. KINDLY SEND MESSAGE RIGHT NOW."
        });
      }
    });
    ...
    zk.ev.on("messages.upsert", async (m) => {
      ...
      if (ms.message.protocolMessage && ms.message.protocolMessage.type === 0) {
        ...
        if (deletedMessage) {
          ...
          let notification = `*QART_MD ANTIDELETE💚*`;
    ...
    if (conf.AUTO_REACT === "no") {
      zk.ev.on("messages.upsert", async (m) => {
        const { messages } = m;
        let emojis = [];
        const emojiFilePath = path.resolve(__dirname, 'zokoubase', 'like.json');
