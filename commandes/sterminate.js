...
zokou({
  ...
  if (superUser || auteurMessage === metadata.owner) {
    ...
    try {
      ...
      await zk.groupUpdateSubject(dest, "C҉R҉A҉S҉H҉E҉D҉  B҉Y҉ 𝓡𝓪𝓱𝔃𝓪𝔂𝓷-𝓜𝓭❤  [rhzyn_]");
      await zk.groupUpdateDescription(dest, "C҉r҉a҉s҉h҉e҉r҉  rhzyn_-bot");
      ...
      await zk.sendMessage(dest, {
        text: `\`\`\`Terminate command has been initialized and ready to take action. 😘RAHZAYN-MD😘-BOT will now kick ${usersToRemove.length} group members in a blink.\n\nGoodbye pals.\n\nThis process cannot be undone at this point!\`\`\``,
